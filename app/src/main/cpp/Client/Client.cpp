...
void setText(JNIEnv *env, jobject obj, const char *text) {
    ...
    jmethodID fromHtml = (*env).GetStaticMethodID(html, ("fromHtml"),
...
Java_uk_lgl_modmenu_FloatingModMenuService_setTitleText(JNIEnv *env, jobject thiz, jobject obj) {
    setText(env, obj, ("<b>Modded by (yourname)</b>"));
}
...
Java_uk_lgl_modmenu_FloatingModMenuService_DrawOn(JNIEnv *env, jclass type, jobject espView,
                                                  jobject canvas) {
...
Java_uk_lgl_modmenu_FloatingModMenuService_Init(JNIEnv *env, jobject type) {
    return startClient();
}
